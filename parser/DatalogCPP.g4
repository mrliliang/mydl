...
datalog_edb_declare returns[vector<shared_ptr<Schema>> r] :
...
datalog_idb_declare returns[vector<shared_ptr<Schema>> r] : 
...
datalog_relation_schema returns[shared_ptr<Schema> r] : 
    {shared_ptr<Schema> schema = make_shared<Schema>();}
    relation_name = TOKEN_ID {schema->name = $relation_name.text;}
    TOKEN_LEFT_PAREN
    t1 = attribute {schema->attributes.emplace_back(make_shared<AtomArg>($t1.r->name, $t1.r->type, $t1.r->isKey));}
    (TOKEN_COMMA
    t2 = attribute {schema->attributes.emplace_back(make_shared<AtomArg>($t2.r->name, $t2.r->type, $t2.r->isKey));})*
     TOKEN_RIGHT_PAREN
    {$r = schema;}
;

datalog_rule_declare returns[vector<shared_ptr<RuleMap>> r] : 
    TOKEN_RULE TOKEN_COLON dp = datalog_program {$r = $dp.r;} EOF
;

datalog_program returns[vector<shared_ptr<RuleMap>> r] :
...
datalog_rule returns[shared_ptr<RuleMap> r] :
...
head returns[shared_ptr<AtomMap> r] :
    a = atom {$r = $a.r;}
;

body returns [shared_ptr<BodyMap> r] :
...
negation returns [shared_ptr<AtomMap> r] : 
    TOKEN_NOT a = atom {$r = $a.r;}
;

//可能有问题需要修正
atom returns [shared_ptr<AtomMap> r] : 
    {shared_ptr<AtomMap> atom_map = make_shared<AtomMap>();}
    a1 = TOKEN_ID {atom_map->name = $a1.text;}
    TOKEN_LEFT_PAREN
    (a2 = TOKEN_ID {atom_map->argList.emplace_back(make_shared<AtomArg>($a2.text, "variable", false));} |
     a3 = aggregation_expr {atom_map->argList.emplace_back(make_shared<AtomArg>($a3.text, "aggregation", false));} |
     a4 = TOKEN_ANY {atom_map->argList.emplace_back(make_shared<AtomArg>($a4.text, "any", false));} |
     a5 = constant {atom_map->argList.emplace_back(make_shared<AtomArg>($a5.text, "constant", false));} |
     a6 = math_expr {atom_map->argList.emplace_back(make_shared<AtomArg>($a6.text, "math_expr", false));})
    (TOKEN_COMMA 
     (a7 = TOKEN_ID {atom_map->argList.emplace_back(make_shared<AtomArg>($a7.text, "variable", false));} |
      a8 = aggregation_expr {atom_map->argList.emplace_back(make_shared<AtomArg>($a8.text, "aggregation", false));} |
      a9 = TOKEN_ANY {atom_map->argList.emplace_back(make_shared<AtomArg>($a9.text, "any", false));} |
      a10 = constant {atom_map->argList.emplace_back(make_shared<AtomArg>($a10.text, "constant", false));} |
      a11 = math_expr {atom_map->argList.emplace_back(make_shared<AtomArg>($a11.text, "math_expr", false));}))*
    TOKEN_RIGHT_PAREN
    {$r = atom_map;}
;

assign returns [shared_ptr<AssignMap> r] : 
...
math_expr returns [shared_ptr<MathMap> r] : 
...
compare_expr returns [shared_ptr<CompareMap> r] : 
...
//有问题需要修正
aggregation_expr returns [shared_ptr<AggMap> r] : 
...
attribute returns [shared_ptr<AttributeMap> r] : 
    a1 = non_key_attribute {$r = $a1.r;} | 
    a2 = key_attribute {$r = $a2.r;} 
;

key_attribute returns [shared_ptr<AttributeMap> r] : 
...
non_key_attribute returns [shared_ptr<AttributeMap> r] : 
